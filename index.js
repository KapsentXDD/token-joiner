...
console.log(gradient.rainbow ("developer by @kapsentcan"))
...
async function doEverything(token, tokens) {
  ...
  client.on("ready", async () => {
    console.log(
      chalk.green("Giris Yapildi ") + gradient.cristal(client.user.tag)
    )
      ...
      .then(async (invite) => {
        await invite
          ...
          .then(async () => {
            console.log(
              chalk.greenBright(
                `Sunucuya Katıldı ${gradient.passion(client.user.tag)}`
              )
            )
            totalJoined++
            ...
            if (client.token === tokens[tokens.length - 1]) {
              console.log(
                `${chalk.magentaBright("[BILGI]")} Joined ${gradient.passion(
                  totalJoined
                ...
                allBoosts.each(async (boost) => {
                  ...
                  setTimeout(async () => {
                    ...
                    console.log(
                      `${chalk.greenBright(
                        "[BAŞARILI]"
                      )} Boost Sunucuya Basıldı ${gradient.cristal(
                        client.user.tag
                      )}}`
                    )
                  }, 500)
